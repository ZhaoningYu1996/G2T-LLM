...
argparser.add_argument('--data_path', type=str, default='data/sampled/qm9/14000/0/samples.json')
argparser.add_argument('--batch_size', type=int, default=30)
argparser.add_argument('--max_new_tokens', type=int, default=1000)
...
for out in tqdm(hf_pipeline(KeyDataset(dataset, "text"), batch_size=args.batch_size, prefix_allowed_tokens_fn=prefix_function, do_sample=True, top_k=50, top_p=0.95, temperature=0.8, pad_token_id=hf_pipeline.tokenizer.eos_token_id, num_workers=args.batch_size), total=len(dataset)):
    try:
    ...
    if smiles:
        ...
        path = args.output_path + 'smiles/10k_17_wo_ke_14000_0.txt'
...
path = args.output_path + 'json/10k_17_wo_ke_14000_0.json'
